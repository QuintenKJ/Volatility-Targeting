...
# en de conventionele volatiliteitstargetingstrategie. In dit script wordt er geen gebruik gemaakt van de Return.portfolio functie 
# (zoals in Conventional_volatilitytargeting_1), hierdoor is het uitvoeren van het script aanzienlijk trager. De bedoeling van dit script 
...
# zelf het script wil gebruiken, dan is het aangeraden om gebruik te maken van Conventional_volatilitytargeting_1, aangezien dit substantieel 
  ...
  # Backtestperiode 
    ...
    # Initialisatie resultaattabellen 
      # Tabel die de waarde bijhoudt van de activa in de geschaalde portefeuille aan het begin van elke dag 
        scaled.bop_value <- xts(matrix(0,nrow = nrow(backtest.asset_returns), ncol = (ncol(backtest.asset_returns) + 3)) , order.by = index(backtest.asset_returns)) 
      ...
      # Tabel die de gewichten bijhoudt van de activa in de geschaalde portefeuille aan het begin van elke dag 
        scaled.bop_weight <- xts(matrix(0,nrow = nrow(backtest.asset_returns), ncol = (ncol(backtest.asset_returns) + 2)) , order.by = index(backtest.asset_returns)) 
      ...
      # Tabel die turnover bijhoudt 
        scaled.turnover <- xts(matrix(0,nrow = nrow(backtest.asset_returns), ncol = ncol(backtest.asset_returns)) , order.by = index(backtest.asset_returns))
    ...
    # Backtest uitvoeren 
      for (d in 1:(nrow(backtest.asset_returns)-1)){ # Lus over de dagen in de backtestperiode (behalve de laatste)
        ...
        if (d %in% endpoints(backtest.asset_returns, on=rebalance.frequency)){ # Herbalanceringsdag 
          ...
          # Waarde van activa aan het begin van de volgende dag is gelijk aan de waarde van de activa aan het einde van deze dag 
...
# Resultaten opslaan voor analyze
