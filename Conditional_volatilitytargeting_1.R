# Dit script bepaalt met behulp van twee backtests de rendementen en portefeuillegewichten voor de niet-geschaalde strategie en de conditionele volatiliteitstargetingstrategie.
...
# Vergeet niet om volgende zaken aan te passen: de gewenste portefeuillegewichten (als decimaal), de plaats om de resultaten op te slaan, de blootstellingslimiet, de volatiliteitslimieten (zie lijn 115 en 116).
...
# Voor de ex post herschaalde versie, waarbij de gerealiseerde volatiliteit van de geschaalde portefeuille gelijk is aan de gerealiseerde volatiliteit van de niet-geschaalde versie, is het noodzakelijk om RV/TV te kennen van de versie zonder vooruitkijkbias, voer daarom eerst dit script en Volatilitytargeting_performance_analysis.R uit zonder vooruitkijkbias. 
# Comment volgende lijnen weg indien je geen ex post herschaling wilt.   
    #expost_scale <- readRDS(paste0(file_map,"performance.rds"))[2,8]
    #file_map <- "Results/P7/L = inf/vol_adj/"
...
# Berekenen gewichten voor volatiliteitstargeting 
  ...
  # Bepalen volatiliteitsregime 
    ...
    # Kwintiel van iedere maand bepalen
      ...
      q1 <- xts(matrix(0, nrow = nrow(unscaled.realized_volatility), ncol = 1), order.by = index(unscaled.realized_volatility)) # Ondergrens
      q2 <- xts(matrix(0, nrow = nrow(unscaled.realized_volatility), ncol = 1), order.by = index(unscaled.realized_volatility)) # Bovengrens
      for (m in 2:nrow(unscaled.realized_volatility)){ # Lus over de maanden, de limieten kunnen pas bepaald worden na 2 maanden 
        q1[m,] <- quantile(unscaled.realized_volatility[c(1:(m-1)),],probs = 0.20) # Onderste volatiliteitsgrens is eerste kwintiel van de maandelijks gerealiseerde volatiliteiten van maand 1 tot de vorige maand  
        q2[m,] <- quantile(unscaled.realized_volatility[c(1:(m-1)),],probs = 0.80) # Bovenste volatiliteitsgrens is het vierde kwintiel van de maandelijks gerealiseerde volatiliteiten van maand 1 tot de vorige maand
    ...
    # Berekenen gewenste blootstelling
      for (m in 1:nrow(target_volatility)){
        if (regime[m,] == 2){ # Gemiddeld volatiliteitsregime
          ...
          # Indien ex post herschaling niet gebruikt wordt, comment onderstaande lijn weg
          #p.rebalance[m,] <- p.rebalance[m,] * (1/expost_scale)  
